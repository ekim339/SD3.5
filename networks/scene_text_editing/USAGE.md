...
```bash
python3 -m pip install -r networks/scene_text_editing/requirements.txt
```
...
```bash
python3 -m networks.scene_text_editing.inference validate_only=true
```

Hydra can print the composed job directly:

```bash
python3 -m networks.scene_text_editing.inference --cfg job
```
...
```bash
cp networks/scene_text_editing/data/text_image_editing/dataset_a/manifest.example.jsonl \
  networks/scene_text_editing/data/text_image_editing/dataset_a/manifest.jsonl
```
...
```bash
python3 -m networks.scene_text_editing.inference \
...
```bash
python3 -m networks.scene_text_editing.download_models dry_run=true
```
...
```bash
python3 -m networks.scene_text_editing.download_models
```
...
python -m pip install -r networks/external/TextCtrl/requirement.txt
...
## Evaluate TextCtrl on COCO-Text

The COCO-Text evaluator selects the first eligible five-character annotation
from each image, expands its bounding box into a word crop, adds deterministic
Gaussian noise, and assigns a random five-character target that differs from
the source. It then runs released TextCtrl SD1.5 and its pretrained ABINet OCR
in one persistent worker process:

```bash
CUDA_VISIBLE_DEVICES=1 \
  python -m networks.scene_text_editing.evaluate_coco_text
```

Settings are in `configs/evaluate_coco_text.yaml` and can be overridden with
Hydra, for example:
...
python -m networks.scene_text_editing.evaluate_coco_text \
  ...
  textctrl.num_inference_steps=50
```

The run is resumable by default. Existing generated crops and OCR predictions
are reused; set `overwrite=true` to regenerate everything. The output includes
the source/noisy/generated crops, the exact evaluation manifest,
`results.csv`, and `summary.json`. CSV columns are `image_title`,
`ground_truth_text`, `ocr_predicted_text`, `ACC`, `NED`, and `CER`.

Metrics use case-insensitive text because the released ABINet checkpoint has a
36-character lowercase-and-digit alphabet. `ACC` is exact-match accuracy,
`NED` is `1 - edit_distance / max(target_length, prediction_length)`, and
`CER` is `edit_distance / target_length`.
...
python3 -m networks.scene_text_editing.train
```
...
```bash
python3 -m unittest discover -s networks/scene_text_editing/tests -v
```
